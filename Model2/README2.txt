...
but a ledger organized as a list with O(N) access time tends to be an
inefficieint structure. The big innovation was to organize the ledger as a
blockchain where each succeeding block has a hash pointer, pointing back 
...
structure called a Merkle Tree: every transaction is hashed, and these 
transactions form the leaves of the tree. Pairs of hashes are grouped 
together and hashed again, to form the next higher level of the tree. The 
single, top level hash is the Merkle Root, which as stored as a field of 
