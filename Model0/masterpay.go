...
func Handle(config map[string]string, conn net.Conn, maxbuf int) {
    ...
    for {
        ...
        fmt.Println("Transaction length ", n)
        if n == maxbuf {
          processed = BuildErrorReply(config, tx)
        } else {
          rerr := methods.VerifyTransaction(tx, false)
          methods.CheckErrorInst(1, rerr, true)
          fmt.Println("Verified? ", rerr)
          processed = ParseTransaction(config, tx, rerr)
        } // endif ParseTrans.
...
func ParseTransaction(conf map[string]string, px structures.Transaction, perr error) string {

  parsee := "undefined"

  switch(px.Ttyp) {
    case "Quit":
      log.Println("Masterpay: got Quit.")
    ...
    case "Balance":
      log.Println("Masterpay: got Balance.")
        if perr == nil {
          firstop := px.Inputs[0]
    ...
    case "Transactions":
      log.Println("Masterpay: got Transactions.")
        if perr == nil {
          firstop := px.Inputs[0]
    ...
    case "PayCoins":
      if perr == nil {
        log.Println("Masterpay: got PayCoins.")
        parsee = Confirm("PayeeReceipt", conf, px, false)
      } else {
        log.Println("Masterpay: PayCoins. failed")
        parsee = "Error"
      } // endif error.

    case "CreateCoins":
      if perr == nil {
        log.Println("Masterpay: got CreateCoins.")
        parsee = Confirm("CreateReceipt", conf, px, true)
      } else {
        log.Println("Masterpay: CreateCoins. Failed.")
        parsee = "Error"
      } // endif error.
