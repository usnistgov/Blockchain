...
func main() {
  ...
  if len(flay) != 1 {
    ...
    fmt.Println("Seq/TransactionId\tTransaction\tVerify\tInputs\tOutputs\tRecip\n")
    for ix := 0; ix < len(ledger); ix++ {
      very := "good."
      tran := ledger[ix]
      verf := methods.VerifyTransaction(tran, false)
      if verf == nil { very = "good." } else { very = "bad." }
      fmt.Printf("%013d/%02d\t%s\t%s\t%d\t%d\t%s\n", tran.Tid, ix, tran.Ttyp, very, len(tran.Inputs), len(tran.Outputs), tran.Outputs[0].Owner[:5])
    } // end for.
