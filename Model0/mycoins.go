...
func mycoins(mykeyfile string) []structures.Coin {
...
func main() {
  ...
  flay := flag.Args()
  if len(flay) == 0 {
    fmt.Println("Usage: mycoins <mypubkeyfile>")
    os.Exit(1)
  } // end if flay.

  structures.PrintCoins(mycoins(flay[0]), "OwnerOut")
  fmt.Printf("Balance for %s is %d\n", flay[0], structures.CoinCount(mycoins(flay[0])))
